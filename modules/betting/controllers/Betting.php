...
class Betting extends ClientsController
{
    ...
    public function payment_g1()
    {
        ...
        $data = [
            ...
            'js' => '',
            'title' => _l("betting"),
            'body' => 'website/' . $this->theme . '/pages/payment',
            'footer' => 'website/' . $this->theme . '/layout/footer',
            // 'category' => $this->betting_model->category(),
            // 'slider' => $this->db->get_where(db_prefix() . 'slider', ['status' => 1])->result(),
            // 'betting' => $this->corn_model->betting(),
    ...
    public function withdraw_g1()
    {
        ...
        $data = [
            ...
            'js' => '',
            'title' => _l("betting"),
            'body' => 'website/' . $this->theme . '/pages/withdraw',
            'footer' => 'website/' . $this->theme . '/layout/footer',
            // 'category' => $this->betting_model->category(),
            // 'slider' => $this->db->get_where(db_prefix() . 'slider', ['status' => 1])->result(),
            // 'betting' => $this->corn_model->betting(),
    ...
    public function payment_g1_ajax(){
        if (!isset($_SESSION['logged_in'])) {
            echo json_encode(['return'=>false,'message'=>'Please Login']);
            die;
        }
        $data = $this->input->post();
        
        $url ='https://nextgaming.live/payment/excute';
        ...
        [
            ...
            "MerchantNotes" => $this->session->userdata('logged_in')->id,
        ]
        ));
        $curl = curl_init();
        ...
        if(strlen($response)>0){
            ...
            echo json_encode(['return'=>$output['status'],'message'=>$output['message'],'url'=>$output['data']['url']]);
        }
        else{
            echo json_encode(['return'=>false,'message'=>'API not give proper data.']);
        }
    }   
    
    public function withdraw_g1_ajax(){
        if (!isset($_SESSION['logged_in'])) {
            echo json_encode(['return'=>false,'message'=>'Please Login']);
            die;
        }
        ...
        $data = $this->input->post();
        $bal = get_settle_balance($_SESSION['logged_in']->id);
        if($data['amount'] > $bal){
            echo json_encode(['return'=>false,'message'=>'you can not withdraw, your balance is '.$bal]);
            die;
        }
        $url ='https://nextgaming.live/payment/withdraw';
        $all = json_encode(array_merge(
        sign(),
        [
            "UserID" => $this->session->userdata('logged_in')->id,
